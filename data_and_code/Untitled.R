...
fullfit <- lm(logPSA ~ Volume + Weight + Age + BPH + factor(SVI) + CP + factor(GS), data=data)
...
################## Final Model ################################
fit <- lm(logPSA ~ Volume + Weight + factor(SVI) + factor(GS) + BPH + Weight:factor(SVI) + Weight:BPH, data = newdata)
summary(fit)
summary(fit)$sigma^2
